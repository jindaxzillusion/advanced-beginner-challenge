        ...
        if self.__display_board.get_handicapped_free_spot().get_number() == spot.get_number():
            ...
            for key in self.__handicapped_spots:
                if self.__handicapped_spots.get(key).is_free():
                    self.__display_board.set_hadicapped_free_spot(
    ...
    def free_spot(self, spot):
        ...
        switcher = {
            ParkingSpotType().HANDICAPPED: self.__free_handicapped_spot_count.update(
                free_spot=self._free_handicapped_spot_count["free_spot"] + 1
            ),
...
class ParkingLot:
    ...
    # similarly exit panel will also use thi object to close parking tickets
    instance = None

    class __OnlyOne:
        def __init__(self, name, address):
            # 1 initinilize variable: read name, addr and parking_rate from database
            # 2 initialize parking floors: read the parking floor map from database, this map should tell how many parking spots are there on each floor. This should also initilize max spot count too.
            # 3 initlize parking spot counts by reading all active tickets from database
            # 4 init entrance and exit panels: read from database
            self.__name = name
            self.__address = address
    ...
    # synchronizing to allow multiple entrances panels to issue a new parking ticket without interfering with each other
        ...
        self.__active_tickets.put(ticket.get_ticket_umber(), ticket)
        self.__lock_release()
        return ticket

    def is_full(self, type):
        # trucks and vans can only be parket in largeSpot
        ...
        # cars can be parked at compact or larget spots
        if type == VehicleType.Car:
            return (self.__compact_spot_count + self.__larget_spot_count) >= (self.__max_compact_count + self.__max_large_count)
        ...
        # increment the parking spot count based on vehicle type
